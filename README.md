...
### Data source
- Primary source: reproducible synthetic JSON payment events generated by the producer service or submitted manually through the UI. `POST /simulate` accepts up to 100,000 events per request. The configured flagged share is exact within the requested base count and is balanced across high amount, high-risk merchant and both rules.
- The optional 15-event same-card burst deliberately uses normal amounts and low-risk merchants. It demonstrates velocity detection independently instead of inflating the flagged share.
...
### User flow
1. Open UI service.
2. Submit one manual payment and see its expected rule before sending, or choose count/flagged percentage and schedule a paced synthetic stream. The API returns `202 Accepted` immediately while generation continues in the background, so large streams do not hold an HTTP connection open.
...
- Ingestion edge and simulation:
  - [Configured rule thresholds](services/producer/app.py#L128)
  - [Transaction ingest endpoint](services/producer/app.py#L135)
  - [Balanced scenario planner](services/producer/app.py#L180)
  - [Background stream worker](services/producer/app.py#L191)
  - [Synthetic stream endpoint](services/producer/app.py#L238)
  - Accepts UI transactions and writes to Kafka; schedules exact, balanced flagged shares plus a velocity-only burst.
...
- Synthetic generation runs as a background task inside the producer pod. The request does not time out, but an in-progress demo stream is not durable across producer pod termination; a production load service would persist jobs in a queue.
...
- Add end-to-end integration tests in CI (current CI covers syntax, producer/processor rule tests, both Helm modes and image builds).
...
- Beyond minimum requirements:
    ...
    - CI pipeline for Python syntax, producer/processor rule tests, default/KEDA Helm rendering and all Docker builds.
